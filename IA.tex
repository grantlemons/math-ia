...
\newcommand{\graphDerDerNormal}{\draw[dashed, darkgray, smooth, thick, samples=100, domain={\leftbound-.05}:{\rightbound+.05}] plot(\x, {\DerDerNormalEq{\x}}) node[darkgray, align=left, right, yshift=-1.5cm, xshift=-3.35cm] {\small\(\sder{f(x)}\)};}
\newcommand{\numline}[1]{
    \draw[black, thick] ({\rightbound+.1}, -0.02) -- ({\leftbound-.1}, -0.02) node[black, align=left, left, yshift=-0.13cm, xshift=.25cm] {\scriptsize \(x\)};
    \foreach \x in  {#1} {
...
%Derivative commands
\newcommand{\der}[1]{\dfrac{d(#1)}{dx}}
\newcommand{\derf}[2]{\dfrac{d(#1)}{dx}(#2)}
\newcommand{\sder}[1]{\dfrac{d^2(#1)}{dx^2}}
\newcommand{\sderf}[2]{\dfrac{d^2(#1)}{dx^2}(#2)}
...
\label{sec:intro}
Integration is the process of calculating the area enclosed by a function (known as the integrand) over an interval.
...
\begin{equation}
    ...
    c(a, b) = \left|\sderf{f(x)}{b} - \sderf{f(x)}{a}\right|
\end{equation}
%
