...
The equation for a trapezoid~\eqref{eqn:trap_rule} is only marginally more complicated than~\eqref{eqn:rect_rule}, but provides significantly more accuracy as seen in Figure~\ref{fig:trap_rule}.
...
One such method is the Composite Simpson's 1/3 rule, which uses the left, right, and, mid-interval values of each subinterval to construct a polynomial using Lagrange Quadratic Interpolation as shown in~\eqref{eqn:lagrange} below.
