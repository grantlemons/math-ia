...
\noindent
Polynomials that fit the function in each subinterval can be found using Lagrange Polynomial Interpolation~(Equation~\ref{eqn:lagrange}).
...
Using the points \(m\) for midpoint, \(a\) for left bound, and \(b\) for right bound in Equation~\ref{eqn:lagrange} produces Equation~\ref{eqn:lagrange_smps} below.
...
\noindent
As \(m\) is the midpoint with the value of \((a + b) / 2\), this can be stated in terms of a and b as shown below in Equation~\ref{eqn:lagrange_smps_ab}.
...
\noindent
The Equation~\ref{eqn:smps_rule} gives an approximate value for the integral value of the interval as a whole using a single polynomial function.
In order to utilize this method for complex integrands, the interval must be split into a number of subintervals for which each will have~\eqref{eqn:smps_rule} applied as shows in Figure~\ref{fig:smps_rule}.
...
\noindent
Which can be further simplified for efficiency to Equation~\ref{eqn:smplfd_composite_smps_rule}:
