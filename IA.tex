...
Since the area of a rectangle is extremely easy to calculate~(\cref{eqn:rect_area}), this method is simple to implement over the entire function by summing the areas of each subinterval as demonstrated in \cref{eqn:rect_rule}.
%
\begin{equation}
    \label{eqn:rect_area}
    A = l \cdot w
...
The equation for the Trapezoid Rule~(\cref{eqn:trap_rule}) is only marginally more complicated than that of the Rectangle Rule~(\cref{eqn:rect_rule}), but fits functions more closely, which provides significantly more accuracy as can be observed in in \cref{fig:trap_rule}.
%
\begin{equation}
    \label{eqn:trap_rule}
...
%
Lagrange Polynomial Interpolation~(\cref{eqn:lagrange}) is a means of constructing a polynomial that passes through a set of points.
...
Like in the Trapezoid and Rectangle rules~(\cref{eqn:trap_rule,eqn:rect_rule}), the interval can be split into smaller subintervals in order to increase accuracy.
...
In order to accomplish this, my function~(\cref{eqn:complexity}) finds the absolute value of the difference between the second derivatives of the two bounds.
%
\begin{equation}
...
\end{equation}
%
In order to use my method, the second derivative of \cref{eqn:normal} must be calculated and hard coded into my program as seen in \cref{lst:rust_functions}.
%
\begin{align*}
