...
\newcommand{\graphSimple}{\draw[darkgray, smooth, thick, samples=100, domain={\leftbound-.05}:{\rightbound+.05}] plot(\x, {((\x+(2/3))^3 + 2*(\x)^2)/3}) node[darkgray, align=left, right, yshift=-0.5cm, xshift=-3.0cm] {\tiny\(f(x)\)};}
...
Both methods are effictive at fitting the curve of a relatively simple function such as a polynomial, but become less effective the more complex the integrand.
