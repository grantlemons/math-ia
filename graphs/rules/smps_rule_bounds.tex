...
\begin{figure}[ht]
    ...
    \resizebox{.7\textwidth}{!}{
        \begin{tikzpicture}[extended line/.style={shorten >=-#1,shorten <=-#1}, spy using outlines={circle, magnification=9, size=2cm, connect spies}]
            ...
            \draw[black, thick] ({\rightbound+.1}, -0.02) -- ({\leftbound-.1}, -0.02) node[black, align=center, left, yshift=-0.13cm, xshift=.5cm] {\tiny \(x\)};
            \draw[darkgray, smooth, thick, samples=100, domain={\leftbound-.05}:{\rightbound+.05}] plot(\x, {\ruleEq{\x}});

            \filldraw [gray] (\a, \fA) circle (1.5pt) node[xshift=-.8cm, yshift=.1cm] {\scriptsize \((x_0, f(x_0))\)};
            \filldraw [gray] (\m, \fM) circle (1.5pt) node[xshift=-.8cm, yshift=.1cm] {\scriptsize \((x_1, f(x_1))\)};
            \filldraw [gray] (\b, \fB) circle (1.5pt) node[xshift=-.8cm, yshift=.1cm] {\scriptsize \((x_2, f(x_2))\)};
        \end{tikzpicture}
    }
    \vspace{-.3cm}
