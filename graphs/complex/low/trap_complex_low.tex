...
\begin{figure}[ht]
    ...
    \resizebox{.7\textwidth}{!}{
        \begin{tikzpicture}[extended line/.style={shorten >=-#1,shorten <=-#1}]
            ...
            \foreach \i in {0, ..., \pgfmathresult}
            {
                ...
                \draw [gray, very thin, pattern=north west lines, pattern color=gray] (BL) -- (BR) -- (TR) -- (TL) -- (BL);
            }
    ...
    \vspace{-.4cm}
    \caption{A sketch showing the ineffectiveness of the trapezoid rule on a difficult function with a low subinterval count}
