...
\begin{gather*}
    ...
    \int_a^b f(x) \,dx \approx h \cdot \sum^{n_s-1}_{j=0}f(x_j) \tageq\label{eqn:rect_rule}
\end{gather*}
%
...
\end{align*}
\begin{align*}
    \int_a^b f(x) \,dx      &\approx    h \cdot \sum^{n_s-1}_{j=0}f(x_j)                            \\
...
This has the advantage that it can better account for slope in the integrand.
