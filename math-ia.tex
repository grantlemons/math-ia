...
%
Applying the Rectangle Rule on \cref{eqn:rule_eqn} with 5 subdivisions (\(n_s\)) (as in \cref{fig:rect_rule}), the calculations are as follows.
\begin{gather*}
...
Instead of picking a single point at which to base the height on, the Trapezoid Rule utilizes the value of the function at both the left and right bounds, and uses them to approximate a trapezoid under the curve.\autocite{num_methods}
...
\begin{gather*}
    h = (b - a) / n_s                                                                                                   \\
    A_{0} = \frac{1}{2} h \left[f(x_0) + f(x_1)\right]                                                                  \\
    \int_a^b f(x) \,dx \approx \frac{1}{2}h \sum^{n_s}_{j=1}[f(x_{j-1}) + f(x_j)] \tageq\label{eqn:trap_rule} 
\end{gather*}
%
